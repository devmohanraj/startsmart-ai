...
function AuthModal({ isOpen, onClose, initialMode = "login", onAuthSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Cold-start detection: if request takes >10s, show "waking up" overlay.
    const coldStartTimer = setTimeout(() => setColdStartWait(true), 10000);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {coldStartWait && (
          <div className="absolute inset-0 bg-gray-900/80 backdrop-blur-sm flex items-center justify-center rounded-2xl z-10">
            <div className="bg-gray-800 rounded-xl shadow-xl p-6 text-center">
              <svg className="w-12 h-12 animate-spin text-indigo-400 mx-auto mb-3" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
              <p className="text-sm font-medium text-gray-300">
                The server is waking up — the first request can take up to a minute. Please hold on, this will complete automatically.
              </p>
            </div>
          </div>
        )}
